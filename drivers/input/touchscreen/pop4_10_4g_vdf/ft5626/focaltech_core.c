...
static int fts_read_Touchdata(struct fts_ts_data *data)
{
    ...
    if(gesture_wakeup_flag && data->suspended)
#else
    if(data->suspended)
#endif
    {
...
int fts_ts_suspend(struct device *dev)
{
...
#endif
    {
        ...
        hall->tp_is_suspend = 1;
#endif
        dev_info(dev, "Entering gesture mode!\n");
        return 0;
    }
...
int fts_ts_resume(struct device *dev)
{
...
#endif
    {
        ...
        input_mt_report_pointer_emulation(data->input_dev, false);
        input_sync(data->input_dev);
        dev_info(dev, "Exit gesture mode!\n");
        return 0;
    }
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (g_ctp_is_founded == 1) {
        dev_info(&client->dev, "fts probe Already founded a right touch device!\n");
        return -1;
    }
    ...
    if (!err && data->ts_pinctrl) {
        ...
        if (err < 0) {
            ...
            goto err_select_pin_state;
        }
    }
#endif

    //reset pin out 0 before power on tp
    if (gpio_is_valid(data->pdata->reset_gpio)) {
    ...
    if (err < 0) {
        ...
        goto err_read_chip_code;
    }

    dev_info(&client->dev, "Device ID = 0x%x\n", reg_value);

    //check the family id(chip code)
    ...
    if (fts_psensor_support_enabled() && data->pdata->psensor_support) {
        ...
        if (!psensor_input_dev) {
            ...
            err = -ENOMEM;
            goto err_malloc_psensor_input_dev;
        }
        ...
        if (err) {
            ...
            goto err_register_psensor_input_dev;
        }
        ...
        if (err)
            goto err_register_sensor_classdev;
    }
#endif
    ...
    err = fts_i2c_read(client, &reg_addr, 1, &reg_value, 1);
    if (err < 0)
        dev_err(&client->dev, "read FTS_REG_FW_VENDOR_ID failed");
    gtp_sensor_id = reg_value;
    dev_info(&client->dev, "gtp_sensor_id = 0x%x\n", gtp_sensor_id);
    ...
    dev_info(&client->dev, "********************Enter CTP Auto Upgrade********************\n");
    ...
    dev_info(&client->dev, "FocalTech touch panel driver probe success!\n");
...
err_malloc_input_dev:
err_read_chip_code:
    fts_gpio_configure(data, false);
err_gpio_config:
    if (pdata->power_on)
...
err_parse_dt:
    ...
    dev_err(&client->dev, "FocalTech touch panel driver probe failed!\n");
    return err;
}
