...
void fts_get_upgrade_array(void)
{
    ...
    if (chip_id != FTS_CHIP_ID || ret < 0) {
        force_update = true;
        chip_id = FTS_CHIP_ID;
        printk("[Focal][Touch] read value fail");
    }

    pr_info("[FTS] %s chip_id = %x\n", __func__, chip_id);
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
    ...
    for (j = 0; j < packet_number; j++) {
        ...
        dev_dbg(&client->dev, "[FTS][%s] bt_ecc = %x \n", __func__, bt_ecc);
        ...
        for (i = 0; i < 30; i++) {
            ...
            dev_dbg(&client->dev, "[FTS][%s] reg_val[0] = %x reg_val[1] = %x \n",
                    __func__, reg_val[0], reg_val[1]);
            msleep(1);
        }
    }
    ...
    if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
        ...
        if (bt_ecc != bt_ecc_check)
            dev_info(&client->dev, "[FTS][%s] Host checksum error bt_ecc_check = %x \n",
                     __func__, bt_ecc_check);
    ...
    for (i = 0; i < 100; i++) {
        ...
        reg_val[0] = reg_val[1] = 0x00;
        fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
        dev_info(&client->dev, "[FTS]--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
    ...
    dev_info(&client->dev, "[FTS]Checksum %X %X \n", reg_val[0], bt_ecc);
    /*********Step 7: reset the new FW***********************/
    FTS_DBG("Step 7: reset the new FW\n");
    auc_i2c_write_buf[0] = 0x07;
    fts_i2c_write(client, auc_i2c_write_buf, 1);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    if (force_update || uc_tp_fm_ver == FTS_REG_FW_VER || uc_tp_fm_ver < uc_host_fm_ver ) {
        msleep(100);
