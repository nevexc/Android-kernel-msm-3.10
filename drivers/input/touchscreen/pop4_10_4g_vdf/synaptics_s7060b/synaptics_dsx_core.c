...
static struct device_attribute attrs[] = {
    ...
    __ATTR(productinfo, (S_IRUGO | S_IWUSR | S_IWGRP),
    synaptics_rmi4_f01_productinfo_show,
    synaptics_rmi4_store_error),
    __ATTR(buildid, (S_IRUGO | S_IWUSR | S_IWGRP),
    synaptics_rmi4_f01_buildid_show,
    synaptics_rmi4_store_error),
    __ATTR(flashprog, (S_IRUGO | S_IWUSR | S_IWGRP),
        ...
        struct synaptics_rmi4_fn *fhandler)
{
    ...
    if (report_gesture_enabled && rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag) {
        retval = synaptics_rmi4_reg_read(rmi4_data,
        ...
        if (retval < 0) {
            dev_err(rmi4_data->pdev->dev.parent, "Read gesture status failed!\n");
            return 0;
        }

        dev_info(rmi4_data->pdev->dev.parent, "Read gesture status = %x\n", detected_gestures);
...
flash_prog_mode:
    ...
    gtp_fw_version = rmi4_data->firmware_id;
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
    ...
    if (!retval && rmi4_data->ts_pinctrl) {
         ...
         */
        retval = pinctrl_select_state(rmi4_data->ts_pinctrl, rmi4_data->pinctrl_state_active);

        if (retval < 0) {
            dev_err(&pdev->dev, "%s: Failed to select %s pinstate %d\n",
...
static int synaptics_rmi4_suspend(struct device *dev)
{
    struct synaptics_rmi4_exp_fhandler *exp_fhandler;
    struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    dev_err(rmi4_data->pdev->dev.parent, "Suspend status is %d %d!\n",
            rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag);

    if (rmi4_data->stay_awake)
        return 0;

    if (rmi4_data->enable_wakeup_gesture && gesture_wakeup_flag) {
        synaptics_rmi4_wakeup_gesture(rmi4_data, true);
        enable_irq_wake(rmi4_data->irq);
        report_gesture_enabled = true;
        goto exit;
    }
...
#endif
    dev_err(rmi4_data->pdev->dev.parent, "Suspend done!\n");
    return 0;
}
...
static int synaptics_rmi4_resume(struct device *dev)
{
    ...
    struct synaptics_rmi4_exp_fhandler *exp_fhandler;
    struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
    dev_err(rmi4_data->pdev->dev.parent, "Resume status is %d %d!\n",
            rmi4_data->enable_wakeup_gesture, gesture_wakeup_flag);
...
#endif
    dev_err(rmi4_data->pdev->dev.parent, "Resume done!\n");
    return 0;
}
