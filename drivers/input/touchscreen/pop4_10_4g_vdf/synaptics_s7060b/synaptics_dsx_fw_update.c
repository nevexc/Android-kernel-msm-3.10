...
//#define FW_COMPATIBLE_MODE
#ifdef FW_COMPATIBLE_MODE
#define FW_IMAGE_NAME_OTHER "fw_update_other.img"
#endif
#define FW_IMAGE_NAME "fw_update.img"
extern unsigned int gtp_cfg_version;

#define DO_STARTUP_FW_UPDATE

static int isFFBMMode = 0;
...
static ssize_t fwu_sysfs_device_config_id_show(struct device *dev,
        struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
    ...
    __ATTR(blocksize, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_block_size_show,
    synaptics_rmi4_store_error),
    __ATTR(fwblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_firmware_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(configblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_configuration_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(dispconfigblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_disp_config_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(permconfigblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_perm_config_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(blconfigblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_bl_config_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(guestcodeblockcount, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_guest_code_block_count_show,
    synaptics_rmi4_store_error),
    __ATTR(writeguestcode, (S_IRUGO | S_IWUSR | S_IWGRP),
    synaptics_rmi4_show_error,
    fwu_sysfs_write_guest_code_store),
    __ATTR(devcfgid, (S_IRUGO | S_IWUSR | S_IWGRP),
    fwu_sysfs_device_config_id_show,
    synaptics_rmi4_store_error),
};
...
static int __init getFFBMMode(char *str) {
    if(!str)
        return 0;
    if(!strncmp("ffbm", str, 4)) {
        isFFBMMode = 1;
    }
    return 1;
}
__setup("androidboot.mode=", getFFBMMode);
...
exit:
    ...
    if (flash_area == NONE) {
        ...
        dev_info(rmi4_data->pdev->dev.parent,
                 ...
                 "UI config only");
        gtp_cfg_version = ((fwu->img.ui_config.data[0] << 8) | (fwu->img.ui_config.data[3]));
    }
...
static int fwu_start_reflash(void)
{
    ...
    if (fwu->image == NULL) {
#ifdef FW_COMPATIBLE_MODE
        if (fwu->config_id[0] == 0x80) {
            retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
                    FW_IMAGE_NAME_OTHER, sizeof(FW_IMAGE_NAME_OTHER),
                    sizeof(FW_IMAGE_NAME_OTHER));
        } else {
            retval = secure_memcpy(fwu->image_name, MAX_IMAGE_NAME_LEN,
...
static ssize_t fwu_sysfs_device_config_id_show(struct device *dev,
        struct device_attribute *attr, char *buf) {
       unsigned int cfgId = 0;
       int retval = fwu_get_device_config_id();
       cfgId = retval >= 0 ? ((fwu->config_id[0] << 8) | (fwu->config_id[3])) : 0;
       return snprintf(buf, PAGE_SIZE, "%u\n", cfgId);
}
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
    ...
    if (retval) {
        ...
        gtp_cfg_version = ((fwu->config_id[0] << 8) | (fwu->config_id[3]));
    }

    dev_info(rmi4_data->pdev->dev.parent,"isFFBMMode = %d\n", isFFBMMode);
#ifdef DO_STARTUP_FW_UPDATE
