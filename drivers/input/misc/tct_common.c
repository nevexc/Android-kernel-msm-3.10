...
int tct_panel_id = 0;
#ifdef KERNEL_FIRST_LCD
static char *first_lcm_name = "nt51021b-1200p-video-dpt";
#endif
#ifdef KERNEL_SECOND_LCD
static char *second_lcm_name = "nt51021b-1200p-video-tdt";
#endif
...
static struct ctp_vendor_info all_ctp_vendor_info[] = {
    ...
    {KERNEL_FIRST_TP, "JDC.T5234", 0x85},
#endif
#ifdef KERNEL_SECOND_TP
    {KERNEL_SECOND_TP, "#132022H1", 0x00},
#endif
...
#ifdef KERNEL_FIRST_LCD
	if(first_lcm_name != NULL) {
		strcpy(lcd_info,first_lcm_name);
		strcat(lcd_info,"_");
		strncat(lcd_info,LK_FIRST_LCD,4);
		strncat(lcd_info,KERNEL_FIRST_LCD,4);
		if(tct_panel_id == 1)
			strcat(lcd_info,"*");
		strcat(lcd_info,"\n");
	}
...
#ifdef KERNEL_SECOND_LCD
	if(second_lcm_name != NULL) {
		strcat(lcd_info,second_lcm_name);
		strcat(lcd_info,"_");
		strncat(lcd_info,LK_SECOND_LCD,4);
		strncat(lcd_info,KERNEL_SECOND_LCD,4);
		if(tct_panel_id == 2)
			strcat(lcd_info,"*");
		strcat(lcd_info,"\n");
	}
